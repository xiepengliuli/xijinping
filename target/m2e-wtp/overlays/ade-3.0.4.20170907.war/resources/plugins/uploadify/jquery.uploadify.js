...
SWFUpload.prototype.initSWFUpload=function(b){
  try {
    ...
    this.initSettings();
    this.loadFlash();
    this.displayDebugInfo()}catch(a){
      delete SWFUpload.instances[this.movieName];throw a
      }};
      SWFUpload.instances={};
      SWFUpload.movieCount=0;SWFUpload.version="2.2.0 2009-03-25";
...
SWFUpload.prototype.fileQueued=function(a){
  ...
  SWFUpload.prototype.uploadProgress=function(a,c,b){a=this.unescapeFilePostParams(a);this.queueEvent("upload_progress_handler",[a,c,b])};
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件"' + file.name + '" 已选择过，是否重新选择？');
					if (!replaceQueueItem) {
...
//=== 文件总数超过2G 不支持上传=========
    var psize=$("#fileSize").val();
    if(psize==""){
      psize=0;
    }
    var tSize=file.size;
    var allSize= parseInt(psize)+parseInt(tSize);
    if(allSize>2147483648){
     alert("总文件大小超过2G!");
     this.cancelUpload(file.id);
     this.queueData.filesCancelled++;
     return false;
   }
   $("#fileSize").val(allSize);
//  ===============================
