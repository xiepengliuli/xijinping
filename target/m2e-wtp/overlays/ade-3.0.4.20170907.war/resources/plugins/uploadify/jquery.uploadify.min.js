...
var swfobject = function() {
  var aq = "undefined", aD = "object", ab = "Shockwave Flash", X = "ShockwaveFlash.ShockwaveFlash", aE = "application/x-shockwave-flash", ac = "SWFObjectExprInst", ax = "onreadystatechange", af = window, aL = document, aB = navigator, aa = false, Z = [ aN ], aG = [], ag = [], al = [], aJ, ad, ap, at, ak = false, aU = false, aH, an, aI = true, ah = function() {
    var a = typeof aL.getElementById != aq && typeof aL.getElementsByTagName != aq
        && typeof aL.createElement != aq, e = aB.userAgent.toLowerCase(), c = aB.platform
        .toLowerCase(), h = c ? /win/.test(c) : /win/.test(e), j = c ? /mac/.test(c) : /mac/
        .test(e), g = /webkit/.test(e) ? parseFloat(e.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1"))
  ...
  function ae(f, d, h, e) {
    ...
    if (a) {
      ...
      var b = ah.ie && ah.win ? "ActiveX" : "PlugIn", c = "MMredirectURL="
          + af.location.toString().replace(/&/g, "%26") + "&MMplayerType=" + b + "&MMdoctitle="
          + aL.title;
  ...
  function aA(e, g, c) {
    ...
    if (a) {
      ...
      if (ah.ie && ah.win) {
        ...
        a.outerHTML = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"' + f + ">" + h
            + "</object>";
        ag[ag.length] = e.id;
        d = aS(e.id);
      } else {
  ...
  function ao(a) {
    ...
    return (b[0] > c[0] || (b[0] == c[0] && b[1] > c[1]) || (b[0] == c[0] && b[1] == c[1] && b[2] >= c[2])) ? true
        : false;
  }
  function az(b, f, a, c) {
    if (ah.ie && ah.mac) {
...
SWFUpload.prototype.getFlashHTML = function() {
  return [ '<object id="', this.movieName, '" type="application/x-shockwave-flash" data="',
      this.settings.flash_url, '" width="', this.settings.button_width, '" height="',
      this.settings.button_height, '" class="swfupload">', '<param name="wmode" value="',
      this.settings.button_window_mode, '" />', '<param name="movie" value="',
      this.settings.flash_url, '" />', '<param name="quality" value="high" />',
...
SWFUpload.prototype.getFlashVars = function() {
  ...
  return [ "movieName=", encodeURIComponent(this.movieName), "&amp;uploadURL=",
      ...
      encodeURIComponent(this.settings.requeue_on_error), "&amp;httpSuccess=",
      encodeURIComponent(d), "&amp;assumeSuccessTimeout=",
      encodeURIComponent(this.settings.assume_success_timeout), "&amp;params=",
      encodeURIComponent(c), "&amp;filePostName=",
      encodeURIComponent(this.settings.file_post_name), "&amp;fileTypes=",
...
SWFUpload.prototype.fileQueued = function(b) {
  ...
  var file = b.size;
  var allSize = parseInt(size) + parseInt(file);
  // 文件总数超过2G 不支持上传
        ...
        if (e[0]) {
          if (e[0] == "*") {
            ...
            c("#" + j.queueID).find(".uploadify-queue-item").each(function() {
              ...
              c(this).find(".data").removeClass("data").html(" - 删除成功");
            ...
            for (var m = 0; m < e.length; m++) {
              i.cancelUpload(e[m]);
              c("#" + e[m]).find(".data").removeClass("data").html(" - 删除成功");
    ...
    onSelect : function(h) {
      ...
      for ( var g in this.queueData.files) {
        ...
        if (f.uploaded != true && f.name == h.name) {
          // alert("现在"+h.name);
          var e = confirm('文件 "' + h.name + '" 已选择过，是否重新选择？');
          if (!e) {
    ...
    onSelectError : function(d, g, f) {
      ...
      if (c.inArray("onSelectError", e.overrideEvents) < 0) {
        ...
        case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
          this.queueData.errorMsg += '\nThe file "' + d.name + '" exceeds the size limit ('
              + e.fileSizeLimit + ").";
          break;
        case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
          ...
          break;
        case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
          this.queueData.errorMsg += '\nThe file "' + d.name + '" is not an accepted file type ('
              + e.fileTypeDesc + ").";
          break;
        }
      }
